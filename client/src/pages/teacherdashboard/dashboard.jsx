            ...
            <div className="space-y-4">
              {recentActivities.map((activity, index) => (
                ...
                >
                  ...
                  <span className={`text-sm px-2 py-1 rounded-full ${activity.status === 'Due soon' ? 'bg-red-100 text-red-600' : 'bg-orange-100 text-orange-600'}`}>
                    {activity.status}
                  </span>
                </div>
              ))}
      ...
      <div className="flex min-h-screen bg-gray-50">
        ...
        <aside className={`fixed left-0 top-0 h-full w-64 bg-white text-gray-700 p-4 shadow-lg transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform z-20`}>
          ...
          <div className="absolute bottom-4 w-full pr-8">
            ...
            <button 
              className="flex items-center gap-2 text-gray-700 hover:bg-red-50 hover:text-red-600 p-2 rounded-lg w-full transition-colors duration-300" 
