...
const VideoPlayer = ({ videoUrl, onBack }) => {
  ...
  const [emailPosition, setEmailPosition] = useState({ top: 0, left: 0 });
  ...
  const generateRandomPosition = () => {
    return {
      top: Math.floor(Math.random() * 80) + 10, // 10-90% of height
      left: Math.floor(Math.random() * 80) + 10  // 10-90% of width
    };
  };
  ...
  useEffect(() => {
    const showEmail = () => {
      ...
      setTimeout(() => setEmailVisible(false), 10000); // Hide after 10 seconds
    };
  ...
  return (
    ...
    >
      ...
      {emailVisible && user?.email && (
        <div 
          className="absolute bg-gray-200/80 px-3 py-1 rounded text-black text-sm"
          style={{
